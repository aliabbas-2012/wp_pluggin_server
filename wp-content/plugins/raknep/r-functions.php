...
function wp_rankie_fetch_rank_whatsmyserp($id){
	...
	if(time('now') < $whatsmysert_disabled_till){
		...
		if(trim($wp_rankie_whatsmyserp_proxy) != ''){
			...
			if(trim($licenseactive) != ''){
				...
				$x='error';
				$url='http://deandev.com/license/proxy_assign.php?purchase='.$wp_rankie_license.'&host='.$_SERVER['HTTP_HOST'];
	...
	$url='http://www.whatsmyserp.com/getSERPresults.php?domainname='.trim(urlencode($keyword_site)).'&googleregional='.$wp_rankie_whatsmyserp_g.'&compdomain1=moz.com&compdomain2=&compdomain3=&keyword='. trim(rawurlencode($keyword)) .'&cached=0&userkeywordid=&prev=2&_=1397911497219';
 ...
 * @param unknown $status fail,success,limit
 */
