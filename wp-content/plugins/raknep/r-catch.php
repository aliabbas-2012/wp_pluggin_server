...
function wp_rankie_function_wp_head_hook(){
	...
	if(stristr($host, 'google.')){
		...
		if(stristr($ref, 'q=')){
			...
			global $wpdb;
			$query="SELECT count(*) as count FROM `wp_rankie_keywords` where keyword ='$searchTerm' and keyword_site='$currentHost' ";
			...
			if($count == 0 ){
				...
				$query="insert into wp_rankie_keywords(keyword,keyword_site,keyword_type) values ('$searchTerm','$currentHost' , 'Auto')";
